...
void showGameModeSel(element_t *elem, window_state_t *state, game_mode_t *gameMode)
{
    ...
    while (waitingForUpdate)
    {
        ...
        if (draw)
        {
            ...
            for (i = 0; i < NUMBER_OF_GAME_MODES; i++)
            {
                al_draw_bitmap(states[i] ? pictures_prs[i] : pictures[i], buttons[i]->x_center - buttons[i]->width + 2, buttons[i]->y_center - 98, ALLEGRO_ALIGN_CENTER);
                ...
                if (buttons[i]->press)
                {
                    ...
                    al_draw_bitmap(elem->modesDescription, buttons[i]->x_center - buttons[i]->width + 2, buttons[i]->y_center - 98, 0);

                    int j;
                    for (j = 0; j < LINES; j++)
                    {
                        al_draw_text(elem->gameModesDescriptionBorder, al_map_rgb(10, 10, 10), buttons[i]->x_center, buttons[i]->y_center + j * 30, ALLEGRO_ALIGN_CENTER, descriptions[i][j]);
                        al_draw_text(elem->gameModesDescription, al_map_rgb(255, 255, 255), buttons[i]->x_center, buttons[i]->y_center + j * 30, ALLEGRO_ALIGN_CENTER, descriptions[i][j]);
                    }
                }
            }
            ...
            for (i = 0; i < NUMBER_OF_GAME_MODES; i++)
            {
                ...
                al_draw_rotated_bitmap(elem->borderLogo, al_get_bitmap_width(elem->borderLogo) / 2.0, al_get_bitmap_height(elem->borderLogo) / 2.0,
                                       buttons[i]->x_center, buttons[i]->y_center - buttons[i]->height, ALLEGRO_PI / 4, 0);

                al_draw_bitmap(pictures_logo[i], buttons[i]->x_center - 50, buttons[i]->y_center - buttons[i]->height - ((i == NOT_EMPTY) ? 50 : 42), 0);
            }
            ...
            for (i = 0; i < 3; i++)
            {
                if (i < numberOfStars)
                {
                    al_draw_text(elem->difficultyBorder, al_map_rgb(0, 0, 0), buttons[DIFFICULTY]->x_center - 40 + i * 40, buttons[DIFFICULTY]->y_center - 18, ALLEGRO_ALIGN_CENTER, "a");
                    al_draw_text(elem->difficulty, al_map_rgb(200, 175, 0), buttons[DIFFICULTY]->x_center - 40 + i * 40, buttons[DIFFICULTY]->y_center - 18, ALLEGRO_ALIGN_CENTER, "a");
                }
                else
                {
                    al_draw_text(elem->difficultyBorder, al_map_rgb(0, 0, 0), buttons[DIFFICULTY]->x_center - 40 + i * 40, buttons[DIFFICULTY]->y_center - 18, ALLEGRO_ALIGN_CENTER, "a");
                    al_draw_text(elem->difficulty, al_map_rgb(100, 100, 100), buttons[DIFFICULTY]->x_center - 40 + i * 40, buttons[DIFFICULTY]->y_center - 18, ALLEGRO_ALIGN_CENTER, "a");
                }
            }
